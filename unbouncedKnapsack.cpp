...
ll val[1005],wt[1005];
ll dp[1005][1005];
ll unbouncedKnapsack(ll w,ll n)
{
    if(n==0 || w<0)
    {
        return 0;
    ...
    if(wt[n-1]>w)
    {
        return dp[n-1][w]=unbouncedKnapsack(w,n-1);
    }
    return dp[n-1][w]=max((val[n-1]+unbouncedKnapsack(w-wt[n-1],n)),unbouncedKnapsack(w,n-1));
}
void solve(int tc)
{
    ...
    cout<<unbouncedKnapsack(w,n)<<endl;
}
int main()
{
