...
ll goriberDP(ll i,ll j)
{
    ...
    if(s[i]==s1[j])
    {
        res=max(res,1LL+(ll)goriberDP(i+1,j+1));
    }
    else
    {
        res=max({res,(ll)goriberDP(i+1,j),(ll)goriberDP(i,j+1),(ll)goriberDP(i+1,j+1)});
    }
    return dp[i][j]=res;
}
void solve(int tc)
